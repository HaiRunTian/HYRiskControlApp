...
import tianchi.com.risksourcecontrol.work.QueryLogWork;

/**
 * Created by Kevin on 2017/12/22.
 */

public class QueryPatrolLogBiz implements IQueryLogBiz {
    ...
    public void queryLog(Map<String, Object> queryCondition, OnLogQueryListener logQueryListener) {
        ...
        OkHttpUtils.postAsync(ServerConfig.URL_QUERY_SAFETY_PATROL_LOG, jsonString, new OkHttpUtils.InsertDataCallBack() {
            @Override
                ...
                try {
                    ...
                    switch (resultCode) {
                        ...
                        case 1:
                            ...
                            List<PatrolLogInfo> patrolLogList = new ArrayList<>();
                            ...
                            for (int i = 0; i < beanJsonArray.length(); i++) {
                                ...
                                PatrolLogInfo patrolLogInfo = GsonUtils.parseJson2Bean(beanObject, PatrolLogInfo.class);
                                patrolLogList.add(patrolLogInfo);
                            }
                            if (patrolLogList.size() > 0) {
                                if (logQueryListener != null) {
                                    logQueryListener.onQuerySucceed(patrolLogList);
                                }
                            }
                            break;
                        default:
