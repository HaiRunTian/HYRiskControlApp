...
public class TunnelTypeFromActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initDatas() {
        ...
        m_edtStakeNum.setText(m_bundle.getString("J_pileNo")+"");//起讫桩号
        m_edtTunnelName.setText(m_bundle.getString("J_tunnelName")+"");//隧道名称
        m_edtForm.setText(m_bundle.getString("J_form")+"");//形式
        m_edtTunnelLength.setText(m_bundle.getString("J_length")+"");//隧道长度
        m_edtDesignSpeed.setText(m_bundle.getString("J_speed")+"");//设计速度
        m_edtClearHeight.setText(m_bundle.getString("J_high")+"");//净高
        m_edtClearWidth.setText(m_bundle.getString("J_width")+"");//净宽


        m_edtGrade.setText(m_bundle.getString("J_scope"));//评分

        String color = m_bundle.getString("J_colorNote");
        switch (color) {
            case "1":
                colorName = "蓝色";
                break;
        ...
        m_edtPosibilityLevel.setText(m_bundle.getString("J_occurPro")+"");//发生可能性等级
        m_edtSeriousesLevel.setText(m_bundle.getString("J_resultGrade")+"");//发生严重性等级
