...
import tianchi.com.risksourcecontrol2.custom.MyTakePicDialog2;
import tianchi.com.risksourcecontrol2.custom.MyToast;
import tianchi.com.risksourcecontrol2.presenter.PatrolSignInPresenter;
...
public class PatrolSignInActivity extends BaseActivity implements AdapterView.OnItemSelectedListener, View.OnClickListener, MyTakePicDialog.OnItemClickListener, IPatrolSignInView, MyTakePicDialog2.OnItemClickListener {
    ...
    TimerTask _timerTask2 = new TimerTask() {
        @Override
        public void run() {
            if (AppInitialization.getInstance().getGPS() != null) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        edtXCoord.setText(String.valueOf(AppInitialization.getInstance().getGPS().getPoint2D().getX()));
                        edtYCoord.setText(String.valueOf(AppInitialization.getInstance().getGPS().getPoint2D().getY()));
                    }
                });
            }
        }
...
//                startLocate();
    ...
    private String getX() {
        ...
        if (AppInitialization.getInstance().getGPS() != null) {
            return String.valueOf(AppInitialization.getInstance().getGPS().getPoint2D().getX());
        } else {
            return "";
        }
    }
    ...
    private String getY() {
        ...
        if (AppInitialization.getInstance().getGPS() != null) {
            return String.valueOf(AppInitialization.getInstance().getGPS().getPoint2D().getY());
        } else {
            return "";
        }
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btnAddPic:
                if (imageItem.size() == 3) {
                    ...
                    MyTakePicDialog2 _takePicDialog = new MyTakePicDialog2();
